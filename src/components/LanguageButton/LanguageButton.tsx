...
const LanguageButton = ({ color, backgroundColor, className }: Props) => {
  const { t } = useTranslation('header')

  const router = useRouter()

  const { ro, ru } = LANGUAGES

  return (
    <Link href={router.pathname} locale={router.locale === ro ? ru : ro}>
      <a className={classNames(styles.languageButtonContainer, 'flex justify-center items-center font-bold', className)}>
        <span style={{ color: color || '#F9F9F9' }} className="absolute">{t('language')}</span>
